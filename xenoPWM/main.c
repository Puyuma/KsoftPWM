...
#define LOW_TIME                                                               \
	(PERIOD_NS / PWM_RANGE * (PWM_RANGE - duty_array[pos].duty_cycle))
...
char *task_name[5] = {"pwmtask_1", "pwmtask_2", "pwmtask_3", "pwmtask_4",
		      "pwmtask_5"};
...
static int xeno_pwm_ioctl(struct rtdm_fd *fd, unsigned int cmd,
			  void __user *arg)
{
	int err = 0, i, pos;
	int *tmp;
	...
	case PWM_INIT: // 0 pin, 1 min, 2 max
		...
		rtdm_printk(KERN_INFO "%d, %d, %d, %d, %s\n",
			    ...
			    task_name[pwm_pin_count]);

		// create RTDM task for pwm task
		err = rtdm_task_init(&task_array[pwm_pin_count],
				     task_name[pwm_pin_count], pwm_task,
				     (void *)pwm_pin_count, TASK_PRIO, 0);
		if (err) {
			printk(
			    KERN_WARNING
			    "failed to create pwm task on rtdm_task_init %d\n",
			    pwm_pin_count);
			break;
		}
...
static struct rtdm_driver xeno_pwm = {
    .profile_info = RTDM_PROFILE_INFO(xenopwm, RTDM_CLASS_EXPERIMENTAL,
				      RTDM_SUBCLASS_GENERIC, XENOPWM_VERSION),
...
static struct rtdm_device xeno_pwm_dev = {
    .driver = &xeno_pwm, .label = PWM_DRIVER,
};
...
int __init xeno_pwm_init(void)
{
	...
	if (ret) {
		printk(KERN_WARNING
		       "failed to register RTDM device for xenopwm\n");
	}
