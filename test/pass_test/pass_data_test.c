...
long pwm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case PWM_SET:
		/*data_tmp = kzalloc(sizeof(PWMset), GFP_KERNEL); //GFP_KERNEL
		to allocate the normal memory
		if(!(pwm_data)) {
			printk(KERN_WARNING "failed to allocate data_tmp memory
		by kzalloc\n");
		}*/
		...
		printk(KERN_ALERT "%d, %d, %d, %d\n", pwm_data->pin1,
		       pwm_data->pin2, pwm_data->pin3, pwm_data->pin4);
	...
	case PWM_INT_ARRAY:
		...
		if(!(tmp)) {
			printk(KERN_WARNING "failed to allocate tmp memory by
		kzalloc\n");
		}*/
...
struct file_operations pwm_fops = {
    .owner = THIS_MODULE, .unlocked_ioctl = pwm_ioctl,
};
...
int __init pwm_init(void)
{
	...
	pwm_data =
	    kzalloc(sizeof(PWMset),
		    GFP_KERNEL); // GFP_KERNEL to allocate the normal memory
	if (!(pwm_data)) {
		printk(KERN_WARNING
		       "failed to allocate pwm_data memory by kzalloc\n");
	}
