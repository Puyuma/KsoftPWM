...
#define LOW_TIME                                                               \
	(PERIOD_US * (PWM_RANGE - duty_array[pos].duty_cycle)) / PWM_RANGE
...
int dev_major;
int dev_minor = 0;
char *name[5] = {"pwmtask_1", "pwmtask_2", "pwmtask_3", "pwmtask_4",
		 "pwmtask_5"};
...
long pwm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_TYPE(cmd) != PWM_MAGIC_NUM)
		return -ENOTTY;
	if (_IOC_NR(cmd) > 4)
		return -ENOTTY;
	...
	case PIN_SET: // 0 pin, 1 type(INPUT or OUTPUT)
		...
		if (tmp[1] != 0 && tmp[1] != 1) {
			printk(KERN_WARNING
			       "wrong pin type input for PIN_SET!\n");
			break;
		}
	...
	case GPIO_SET: // 0 pin, 1 status(HIGH or LOW)
		...
		if (tmp[1] != 0 && tmp[1] != 1) {
			printk(KERN_WARNING
			       "wrong pin type input for GPIO_SET!\n");
			break;
		}
	...
	case PWM_INIT: // 0 pin, 1 min, 2 max
		...
		pwm_tsk[pwm_pin_count] = kthread_create(
		    pwm_task, (void *)pwm_pin_count, name[pwm_pin_count]);
		...
		printk(KERN_INFO "%d, %d, %d, %d",
		       duty_array[pwm_pin_count].pin,
		       duty_array[pwm_pin_count].min,
		       duty_array[pwm_pin_count].max,
		       duty_array[pwm_pin_count].duty_cycle);
...
struct file_operations pwm_fops = {.owner = THIS_MODULE,
...
int __init pwm_init(void)
{
	...
	if (!(duty_array)) {
		printk(KERN_WARNING
		       "failed to allocate duty_array memory by kzalloc\n");
	}

	// allocate pwm_tsk array memory
	pwm_tsk =
	    kzalloc(sizeof(struct task_struct *) * MAX_PWM_NUM, GFP_KERNEL);
	if (!(pwm_tsk)) {
		printk(KERN_WARNING
		       "failed to allocate pwm_tsk array memory by kzalloc\n");
	}
	...
	if (result < 0) {
		printk(KERN_WARNING
		       "failed to allocate char device for softPWM\n");
		return result;
	}
	...
	cdevp = kzalloc(sizeof(struct cdev),
			GFP_KERNEL); // GFP_KERNEL to allocate the normal memory
	if (!(cdevp)) {
		printk(KERN_WARNING
		       "failed to allocate device memory by kzalloc\n");
		goto failed;
	}
